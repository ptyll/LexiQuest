...
public class MultiplayerController : ControllerBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await _matchHistoryService.GetMatchResultAsync(matchId, userId.Value, cancellationToken);
        if (result == null)
        {
            return NotFound();
        }
